...
use crate::{
    ...
    network::{nic::Nic, structs::MacAddress},
...
impl E1000e<'_> {
    ...
    const RCTL_BSIZE_256: u32 = (0b11 << 16) | (0b0 << 25);
    const RCTL_BSIZE_512: u32 = (0b10 << 16) | (0b0 << 25);
    const RCTL_BSIZE_1024: u32 = (0b01 << 16) | (0b0 << 25);
    const RCTL_BSIZE_2048: u32 = (0b00 << 16) | (0b0 << 25);
    ...
    fn enable_int(&self) {
        ...
        let msix_capable = crate::pci::with_pci_handler(|handler| {
            crate::pci::msix::find_msix_cap(handler, &self.pci_device)
        });
        let msi_capable = if msix_capable.is_none() {
            crate::pci::with_pci_handler(|handler| {
                crate::pci::msi::find_msi_cap(handler, &self.pci_device)
            })
        } else {
            None
        };
    ...
    fn configure_descriptors(
        ...
        self.write_reg(Self::TIPG, 0x00602006); // See Section 10.2.6.2 Note
    }
