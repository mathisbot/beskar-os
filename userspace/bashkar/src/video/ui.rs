...
pub const SHADOW_GREEN: PixelComponents = PixelComponents::new(0x00, 0xAA, 0x00, 0xFF);
...
pub fn draw() {
    screen::with_ui_surface(|fb| {
        ...
        };
        {
            ...
            let mut canvas = AsciiCanvas::new(info, pixels, theme);
        ...
        } // pixels borrow ends here

        if let Err(e) = fb.flush_all() {
            beskar_lib::println!("Failed to flush framebuffer: {}", e);
        }
    });

    // Initialize the TTY surface now that we know the layout
    let layout = layout();
    let tty_x = layout.inner_left_col.saturating_mul(layout.cell_width_px);
    let tty_y = layout.text_start_row.saturating_mul(layout.cell_height_px);
    let tty_width = layout.inner_width_cols.saturating_mul(layout.cell_width_px);
    let tty_height = layout
        .inner_height_rows
        .saturating_mul(layout.cell_height_px);

    screen::init_tty_surface(tty_x, tty_y, tty_width, tty_height);
}
